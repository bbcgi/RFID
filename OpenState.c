	...
	while(1){
		...
		switch(command){
				...
				{	
					...
					switch(WriteCommand->ManBank_Write){
						case RESERVED_MEM_BANK :
							...
							if(WriteCommand->WordPtr_Write==1){
								YLED=LED_ON;
							}
							break;
						case EPC_MEM_BANK :
							word_pointer=(unsigned short*)(&(EpcMB.EPC_DATA));
							break;
						case TID_MEM_BANK:
			...
			case KILL_HEADER:
				...
				{
					if(openSateKillFirstDone){
						openSateKillFirstDone=0;
			...
			case BLOCK_WRITE_HEADER :
				WriteCommand=(MyWrite*)buffer;
				tempShort=buffer[ReciveByte-2];
				tempShort<<=8;
				tempShort+=buffer[ReciveByte-1];
				if(tempShort==Handler)// check && /*valid memory access*/
				{	
					halRFSetRxTxOff(RF_TX, &RF_SETTINGS, &RF_CALDATA);
					switch(buffer[1]){
						case RESERVED_MEM_BANK :
					...
					word_pointer+=buffer[2];
					for(i=0;i<buffer[3];i++){
						*word_pointer=(buffer[4+i*2]<<8)+buffer[4+i*2+1];
						word_pointer++;
					}
					word_pointer=0x00;
